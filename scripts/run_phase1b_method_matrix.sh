...
MAX_NEW_TOKENS_CHOICE="${MAX_NEW_TOKENS_CHOICE:-8}"
MAX_NEW_TOKENS_GENERATE="${MAX_NEW_TOKENS_GENERATE:-96}"
...
eval_mode_for_method() {
  case "$1" in
    direct|soft|latent) echo "binary_choice" ;;
    cot|masked_cot) echo "generate" ;;
    *) echo "Unknown method: $1" >&2; exit 2 ;;
  esac
}

max_tokens_for_eval_mode() {
  case "$1" in
    binary_choice) echo "${MAX_NEW_TOKENS_CHOICE}" ;;
    generate) echo "${MAX_NEW_TOKENS_GENERATE}" ;;
    *) echo "Unknown eval mode: $1" >&2; exit 2 ;;
  esac
}

for method in ${METHODS}; do
  eval_mode="$(eval_mode_for_method "${method}")"
  max_new_tokens="$(max_tokens_for_eval_mode "${eval_mode}")"
